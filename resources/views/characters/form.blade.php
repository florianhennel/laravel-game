...
<x-modal show="true" name="edit" class="">
    <div class="flex flex-col items-center p-2">
        ...
        <form @yield('formMethodAndAction') class="mt-6 space-y-6">
            @csrf
            @yield('method')
            <div>
                <x-input-label for="name" :value="__('Name')" />
                <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', @yield('characterName'))"
                    required autofocus autocomplete="name" />
                <x-input-error class="mt-2" :messages="$errors->get('name')" />
            </div>
            <div>
                <x-input-label for="defence" :value="__('Defence')" />
                <x-number-input id="defence" name="defence" type="number" class="mt-1 block w-full" :value="old('defence', @yield('characterDefence'))"
            ...
            <div>
                ...
                <x-number-input id="strength" name="strength" type="number" class="mt-1 block w-full"
                    :value="old('strength', @yield('characterStrength'))" required autofocus autocomplete="strength" />
            ...
            <div>
                ...
                <x-number-input id="accuracy" name="accuracy" type="number" class="mt-1 block w-full"
                    :value="old('accuracy', @yield('characterAccuracy'))" required autofocus autocomplete="accuracy" />
            ...
            <div>
                ...
                <x-number-input id="magic" name="magic" type="number" class="mt-1 block w-full"
                    :value="old('magic', @yield('characterMagic'))" required autofocus autocomplete="magic" />
